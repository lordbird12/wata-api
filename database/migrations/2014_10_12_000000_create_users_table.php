...
class CreateUsersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('users', function (Blueprint $table) {
            ...
            $table->string('birthday', 255)->charset('utf8');
            $table->integer('age')->unsigned()->index();
            $table->enum('sex', ['M', 'F'])->charset('utf8')->default('M');
            $table->enum('type', ['0', '1', '2', '3'])->charset('utf8')->default('0');
            $table->string('phone', 10)->charset('utf8');
            $table->string('line', 255)->charset('utf8');
            $table->string('email')->unique();
            $table->string('position', 255)->charset('utf8');
            $table->string('department', 255)->charset('utf8');
            $table->string('organization', 255)->charset('utf8');
